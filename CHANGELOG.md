...
- A resubscribe cut short by the socket dropping no longer sends a second `Err` to the consumer, restoring the documented one `Err` per disconnect ([#45](https://github.com/aursen-labs/spume/pull/45)).
- A non-2xx response carrying a parseable `result` is no longer treated as success — a regression from [#41](https://github.com/aursen-labs/spume/pull/41) ([#43](https://github.com/aursen-labs/spume/pull/43)).
